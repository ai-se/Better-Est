...
def rdivDemo(data, isLatex=False, globalMinMax=False, high=100, low=0, bigger_better = False):
  if type(data)==type([]):
  ...
  if isLatex:
    ...
    if bigger_better:
      upper = max([r[0] for r in ranks])
      ranks = [(upper-r[0],r[1],r[2]) for r in ranks]
    all = []
...
#     ''')
    ...
    for rank, __, x in sorted(ranks):
      ...
      print ('%4s , %12s ,    %0.2f  ,  %0.2f ' %
             (rank + 1, x.name, x.median(), x.spread())) + \
